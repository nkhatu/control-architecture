...
def create_app(settings: AppSettings | None = None) -> FastAPI:
    ...
    service = PolicyEngine(control_plane_config, app_name=app_settings.app_name)
    ...
    app = FastAPI(
        title="Policy Engine",
        description="Deterministic policy decision engine for the domestic money movement PoC.",
        version="0.1.0",
        lifespan=lifespan,
    )

    def get_service() -> PolicyEngine:
        return service
    ...
    @app.get("/metadata")
    def metadata(service: PolicyEngine = Depends(get_service)) -> dict[str, object]:
    ...
    def evaluate_intake(
        ...
        service: PolicyEngine = Depends(get_service),
